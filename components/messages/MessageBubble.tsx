            ...
            <CardContent className="p-4 space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    For: {relatedVehicle?.title || "Vehicle"}
                  </p>
                </div>
                <TestDriveStatusBadge status={details.status} />
              </div>
              <div className="text-sm space-y-1 text-slate-700">
                <p><strong>Date:</strong> {format(new Date(details.preferred_date), 'EEE, MMM d, yyyy')}</p>
                <p><strong>Time:</strong> {details.preferred_time}</p>
              ...
              {details.status === 'pending' && !isMyMessage && (
                <div className="flex gap-2 pt-2 border-t border-slate-200/80">
                  <Button size="sm" onClick={() => onApproveTestDrive(message.id)} className="flex-1 bg-emerald-500 hover:bg-emerald-600">
                    Approve
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => onDeclineTestDrive(message.id)} className="flex-1">
                    Decline
                  </Button>
                </div>
              )}
