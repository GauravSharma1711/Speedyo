...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!vehicleId || !buyerId || !sellerId || !transfer_type || !admin_notes || !user_facing_notes) {
      return NextResponse.json(
        { error: "vehicleId, buyerId, sellerId, admin_notes, user_facing_notes, and transfer_type are required" },
        { status: 400 }
      );
    }
